...
class GraphPair(object):
    def __init__(self, y_true_dict_list=None, ds_true=None, g1=None, g2=None,
                 check_1_1=False, running_time=None):
    ...
    def get_ds_pred(self):
        if not hasattr(self, 'ds_pred'):
            raise ValueError('Must call assign_ds_pred before calling this')
        return self.ds_pred
...
def _convert_to(x, format):
    if type(x) is torch.Tensor and format == 'numpy':
        return x.detach().cpu().numpy()
    elif type(x) is np.ndarray and 'torch' in format:
