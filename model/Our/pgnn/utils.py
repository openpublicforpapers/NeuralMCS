...
def get_random_anchorset(n, num_anchors, c=0.5):
    m = num_anchors #int(np.log2(n))
    copy = int(c*m)
    anchorset_id = []
    for i in range(m):
        anchor_size = max(1,int(n/np.exp2(i + 1)))
        for j in range(copy):
            anchorset_id.append(np.random.choice(n,size=anchor_size,replace=False))
    return anchorset_id

def get_num_anchors(ns):
    ms = [np.log2(n) for n in ns]
    m = sum(ms)/len(ms)
    return int(m)
...
def preselect_anchor(dists, num_nodes, num_anchors, layer_num=2, anchor_num=8, anchor_size_num=4, device='cpu'):
    anchor_set = []
    anchor_num_per_size = anchor_num//anchor_size_num
    for i in range(anchor_size_num):
        anchor_size = 2**(i+1)-1
        anchors = np.random.choice(num_nodes, size=(layer_num,anchor_num_per_size,anchor_size), replace=True)
        anchor_set.append(anchors)
    anchor_set_indicator = np.zeros((layer_num, anchor_num, num_nodes), dtype=int)
    anchorset_id = get_random_anchorset(num_nodes,num_anchors,c=1)
